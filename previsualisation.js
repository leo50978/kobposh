...
class PrevisualisationCV extends HTMLElement {
    constructor() {
        ...
        this.hotspotDragMoveHandler = null;
        this.hotspotDragUpHandler = null;
        this.lastPageHeightAdjustPx = 0;
        this.maxLastPageHeightAdjustPx = 2200;
        this.lastPageResizeState = null;
        this.lastPageResizeMoveHandler = null;
        this.lastPageResizeUpHandler = null;
    ...
    disconnectedCallback() {
        ...
        this.endHotspotDrag({ commit: false });
        this.endLastPageResize({ commit: false });
        ...
        this.innerHTML = `
            ...
            <section class="pv-root relative h-screen overflow-y-auto p-2 font-[Poppins] sm:p-3">
                <div class="pv-card mx-auto flex max-w-[1240px] flex-col gap-3">
                    <div class="pv-topbar flex flex-wrap items-center justify-between gap-2 p-2 sm:p-3">
                        <div>
                            ...
                            <div class="hidden" aria-hidden="true">
                                <div id="pv-preview" class="pv-paper pv-print grid min-h-[1123px] w-[794px] grid-cols-[30%_70%] border border-slate-300 bg-white text-[#222222] shadow-sm">
                                    ...
                                    <main class="pv-right flex min-w-0 flex-col gap-6 bg-white p-10 text-[#222222]">
                                        ...
                                        <section class="pv-r-section flex min-h-0 flex-col" data-autofit-box data-edit-step="educations">
                                        ...
                                        <section class="pv-r-section flex min-h-0 flex-col" data-autofit-box data-edit-step="experiences">
                                        ...
                                        <section class="pv-r-section flex min-h-0 flex-col" data-autofit-box>
                                            <h4 class="pv-r-title m-0 mb-[15px] text-[16px] font-semibold uppercase tracking-[3px]">Competences</h4>
                                            <div class="pv-r-line mb-3.5 h-1 w-[50px] rounded bg-[#2f3e4e]"></div>
                                            <div class="pv-skills-grid grid grid-cols-2 items-start gap-[18px]" data-autofit-box>
    ...
    bindEvents() {
        this.addEventListener('pointerdown', (event) => {
            ...
            const resizeHandle = target.closest('[data-last-page-resize-handle="1"]');
            if (resizeHandle) {
                this.startLastPageResize(resizeHandle, event);
                return;
            }

            const hotspot = target.closest('[data-style-hotspot="1"]');
            if (!hotspot || !this.moveModeEnabled) return;
            const styleKey = String(hotspot.getAttribute('data-style-key') || '').trim();
            if (!styleKey) return;
            this.closeInlineTextEditor({ applyChanges: false });
            this.selectStyleTargetByKey(styleKey, false);
            this.startHotspotDrag(hotspot, event);
        });

        this.addEventListener('click', (event) => {
            const insideSelectionScope = Boolean(event.target.closest('[data-style-hotspot="1"], [data-style-editor], [data-style-move-controls="1"], [data-inline-text-editor="1"], [data-last-page-resize-handle="1"], [data-page-meta-hotspot="1"]'));
            ...
            const pageMetaHotspot = event.target.closest('[data-page-meta-hotspot="1"]');
            if (pageMetaHotspot) {
                event.preventDefault();
                event.stopPropagation();
                const metaType = pageMetaHotspot.getAttribute('data-page-meta-type') || '';
                this.openPageMetaEditor(metaType, pageMetaHotspot);
                return;
            }
    ...
    normalizePageMetaType(value) {
        const key = String(value || '').trim().toLowerCase();
        if (key === 'header' || key === 'footer') return key;
        return '';
    }

    normalizePageMetaText(value) {
        return String(value ?? '')
            .replace(/\r/g, '')
            .slice(0, 320);
    }

    syncPageMetaStateFromData(resolvedData = null) {
        const source = (this.data && typeof this.data === 'object') ? this.data : {};
        if (Object.prototype.hasOwnProperty.call(source, 'page_header')) {
            this.pageHeaderText = this.normalizePageMetaText(source.page_header);
        } else if (typeof this.pageHeaderText !== 'string') {
            this.pageHeaderText = this.normalizePageMetaText(resolvedData?.page_header || '');
        }

        if (Object.prototype.hasOwnProperty.call(source, 'page_footer')) {
            this.pageFooterText = this.normalizePageMetaText(source.page_footer);
        } else if (typeof this.pageFooterText !== 'string') {
            this.pageFooterText = this.normalizePageMetaText(resolvedData?.page_footer || '');
        }
    }

    getPageMetaText(type) {
        const key = this.normalizePageMetaType(type);
        if (key === 'header') return this.normalizePageMetaText(this.pageHeaderText);
        if (key === 'footer') return this.normalizePageMetaText(this.pageFooterText);
        return '';
    }

    setPageMetaText(type, value) {
        const key = this.normalizePageMetaType(type);
        if (!key) return false;
        const nextText = this.normalizePageMetaText(value);
        if (key === 'header') {
            if (nextText === this.pageHeaderText) return false;
            this.pageHeaderText = nextText;
            if (this.data && typeof this.data === 'object') this.data.page_header = nextText;
            return true;
        }
        if (nextText === this.pageFooterText) return false;
        this.pageFooterText = nextText;
        if (this.data && typeof this.data === 'object') this.data.page_footer = nextText;
        return true;
    }

    updatePreview() {
        const data = this.resolveDataWithDefaults();
        this.syncPageMetaStateFromData(data);
    ...
    openInlineTextEditor(styleKey, anchorNode = null) {
        ...
        this.inlineEditorState = { mode: 'style-target', styleKey: cleanKey, layer, box, input };
        return true;
    }

    openPageMetaEditor(type, anchorNode = null) {
        const metaType = this.normalizePageMetaType(type);
        ...
        box.className = 'absolute z-[7] rounded-xl border border-slate-300 bg-white p-2 shadow-xl';
        box.setAttribute('data-inline-text-editor', '1');
        const preferredWidth = Math.max(240, Math.min(420, anchorRect.width + 80));
        const editorWidth = Math.max(200, Math.min(preferredWidth, Math.max(200, layerRect.width - 16)));
        const leftPx = Math.max(8, Math.min(layerRect.width - editorWidth - 8, anchorRect.left - layerRect.left));
        const preferredTop = (anchorRect.top - layerRect.top) - 126;
        const fallbackTop = (anchorRect.bottom - layerRect.top) + 8;
        const topPx = preferredTop >= 8
        ...
        box.innerHTML = `
            ...
            <p class="mt-1 text-[10px] text-slate-500">Laissez vide si vous ne voulez rien afficher.</p>
        `;
        layer.appendChild(box);

        const input = box.querySelector('[data-inline-input]');
        if (!(input instanceof HTMLTextAreaElement)) return false;
        input.value = this.getPageMetaText(metaType);
        input.placeholder = metaType === 'header'
            ? 'Entrez un header de page (optionnel)'
            : 'Entrez un footer de page (optionnel)';
        input.addEventListener('keydown', (event) => {
            if (event.key === 'Escape') {
                event.preventDefault();
                this.closeInlineTextEditor({ applyChanges: false });
        ...
        this.inlineEditorState = { mode: 'page-meta', metaType, layer, box, input };
        return true;
    }
    ...
    commitInlineTextEditor() {
        ...
        if (state.mode === 'page-meta') {
            this.setPageMetaText(state.metaType, nextText);
            this.closeInlineTextEditor({ applyChanges: false });
    ...
    clampMoveOffsetPx(value) {
        const parsed = Number(value);
        if (!Number.isFinite(parsed)) return 0;
        return Math.max(-this.maxMoveOffsetPx, Math.min(this.maxMoveOffsetPx, Math.round(parsed)));
    }

    applyLiveStyleForKey(styleKey, preview = null) {
        const cleanKey = String(styleKey || '').trim();
        if (!cleanKey) return;
        const host = preview || this.querySelector('#pv-preview');
        if (!host) return;
        const target = this.getStyleTargetElement(cleanKey, host);
        if (!target) return;
        const style = this.elementStyleMap[cleanKey] || {};
        this.applyElementStyle(target, style);
    }

    startHotspotDrag(hotspot, event) {
        if (!hotspot || !(event instanceof PointerEvent)) return;
        if (!this.moveModeEnabled) return;

        const styleKey = String(hotspot.getAttribute('data-style-key') || '').trim();
        if (!styleKey) return;
        const style = this.elementStyleMap[styleKey] || {};
        this.elementStyleMap[styleKey] = style;

        this.endHotspotDrag({ commit: false });

        const scaleRaw = Number(hotspot.dataset.hotspotScale || 1);
        const scale = Number.isFinite(scaleRaw) && scaleRaw > 0 ? scaleRaw : 1;
        const startOffsetX = Number(style.offsetXPx || 0);
        const startOffsetY = Number(style.offsetYPx || 0);
        ...
        this.hotspotDragState = {
            ...
            startOffsetX: Number.isFinite(startOffsetX) ? startOffsetX : 0,
            startOffsetY: Number.isFinite(startOffsetY) ? startOffsetY : 0,
            scale
        };

        const escapedKey = styleKey.replace(/"/g, '\\"');
        this.querySelectorAll(`[data-style-hotspot="1"][data-style-key="${escapedKey}"]`).forEach((node) => {
            node.classList.add('is-dragging');
        });

        this.hotspotDragMoveHandler = (moveEvent) => this.onHotspotDragMove(moveEvent);
        this.hotspotDragUpHandler = (upEvent) => this.endHotspotDrag({ event: upEvent, commit: true });
        window.addEventListener('pointermove', this.hotspotDragMoveHandler);
        window.addEventListener('pointerup', this.hotspotDragUpHandler);
        window.addEventListener('pointercancel', this.hotspotDragUpHandler);
        ...
        document.body.classList.add('pv-drag-cursor');
        event.preventDefault();
        event.stopPropagation();
    }

    onHotspotDragMove(event) {
        const state = this.hotspotDragState;
        ...
        const nextX = this.clampMoveOffsetPx(state.startOffsetX + deltaX);
        const nextY = this.clampMoveOffsetPx(state.startOffsetY + deltaY);

        const style = this.elementStyleMap[state.styleKey] || {};
        if (nextX !== 0) style.offsetXPx = String(nextX);
        else delete style.offsetXPx;
        if (nextY !== 0) style.offsetYPx = String(nextY);
        else delete style.offsetYPx;
        this.elementStyleMap[state.styleKey] = style;

        this.applyLiveStyleForKey(state.styleKey);
        this.syncVisibleHotspotsWithOffsets({ skipSelectionRefresh: true });
        event.preventDefault();
    }

    endHotspotDrag(options = {}) {
        const state = this.hotspotDragState;
        if (!state) return;
        const event = options?.event;
        if (event instanceof PointerEvent && event.pointerId !== state.pointerId) return;

        if (this.hotspotDragMoveHandler) {
            window.removeEventListener('pointermove', this.hotspotDragMoveHandler);
            this.hotspotDragMoveHandler = null;
        }
        if (this.hotspotDragUpHandler) {
            window.removeEventListener('pointerup', this.hotspotDragUpHandler);
            window.removeEventListener('pointercancel', this.hotspotDragUpHandler);
            this.hotspotDragUpHandler = null;
        }

        const escapedKey = state.styleKey.replace(/"/g, '\\"');
        this.querySelectorAll(`[data-style-hotspot="1"][data-style-key="${escapedKey}"]`).forEach((node) => {
            node.classList.remove('is-dragging');
        });
        document.body.classList.remove('pv-drag-cursor');
        this.hotspotDragState = null;
    ...
    clampLastPageHeightAdjust(value, basePageHeightPx, marginPx = 0) {
        const parsed = Number(value);
        const baseHeight = Math.max(1, Math.floor(Number(basePageHeightPx) || 1));
        const minimumHeight = Math.max((marginPx * 2) + 120, Math.round(baseHeight * 0.48));
        const minAdjust = minimumHeight - baseHeight;
        const maxAdjust = this.maxLastPageHeightAdjustPx;
        const safeValue = Number.isFinite(parsed) ? Math.round(parsed) : 0;
        return Math.max(minAdjust, Math.min(maxAdjust, safeValue));
    }

    startLastPageResize(handle, event) {
        if (!handle || !(event instanceof PointerEvent)) return;
        const pageFrame = handle.closest('[data-page-frame="1"]');
        ...
        const baseCanvasHeight = Math.max(1, Math.floor(Number(pageFrame.dataset.basePageCanvasHeight || 0) || 1));
        const pageCanvasWidth = Math.max(1, Math.floor(Number(pageFrame.dataset.pageCanvasWidth || 0) || 794));
        const pageCanvasHeight = Math.max(1, Math.floor(Number(pageFrame.dataset.pageCanvasHeight || 0) || baseCanvasHeight));
        const currentAdjust = pageCanvasHeight - baseCanvasHeight;
        const scale = pageCanvasWidth / frameRect.width;
        const startAdjust = Number.isFinite(currentAdjust) ? currentAdjust : this.lastPageHeightAdjustPx;
        this.lastPageHeightAdjustPx = startAdjust;

        this.lastPageResizeState = {
            pointerId: event.pointerId,
            pageFrame,
            startClientY: event.clientY,
            startAdjust,
            baseCanvasHeight,
            pageCanvasWidth,
            startFrameHeightPx: frameRect.height,
            scale
        };

        this.lastPageResizeMoveHandler = (moveEvent) => this.onLastPageResizeMove(moveEvent);
        this.lastPageResizeUpHandler = (upEvent) => this.endLastPageResize({ event: upEvent, commit: true });
        window.addEventListener('pointermove', this.lastPageResizeMoveHandler);
        window.addEventListener('pointerup', this.lastPageResizeUpHandler);
        window.addEventListener('pointercancel', this.lastPageResizeUpHandler);
        ...
        pageFrame.classList.add('pv-last-page-resizing');
        document.body.classList.add('pv-drag-cursor');
        event.preventDefault();
        event.stopPropagation();
    }

    onLastPageResizeMove(event) {
        const state = this.lastPageResizeState;
        if (!state || !(event instanceof PointerEvent)) return;
        if (event.pointerId !== state.pointerId) return;
        const scale = Number.isFinite(state.scale) && state.scale > 0 ? state.scale : 1;
        const deltaCanvasPx = (event.clientY - state.startClientY) * scale;
        const nextAdjust = this.clampLastPageHeightAdjust(
            state.startAdjust + deltaCanvasPx,
            state.baseCanvasHeight
        );
        this.lastPageHeightAdjustPx = nextAdjust;

        const displayDelta = (nextAdjust - state.startAdjust) / scale;
        const nextDisplayHeight = Math.max(120, state.startFrameHeightPx + displayDelta);
        state.pageFrame.style.height = `${nextDisplayHeight.toFixed(2)}px`;
        state.pageFrame.style.overflow = 'hidden';
        event.preventDefault();
    }

    endLastPageResize(options = {}) {
        const state = this.lastPageResizeState;
        if (!state) return;
        const event = options?.event;
        if (event instanceof PointerEvent && event.pointerId !== state.pointerId) return;

        if (this.lastPageResizeMoveHandler) {
            window.removeEventListener('pointermove', this.lastPageResizeMoveHandler);
            this.lastPageResizeMoveHandler = null;
        }
        if (this.lastPageResizeUpHandler) {
            window.removeEventListener('pointerup', this.lastPageResizeUpHandler);
            window.removeEventListener('pointercancel', this.lastPageResizeUpHandler);
            this.lastPageResizeUpHandler = null;
        }

        state.pageFrame.classList.remove('pv-last-page-resizing');
        state.pageFrame.style.removeProperty('height');
        state.pageFrame.style.removeProperty('overflow');
        document.body.classList.remove('pv-drag-cursor');
        this.lastPageResizeState = null;

        if (options?.commit) {
            this.schedulePaginatedPreviewRender({ delayMs: 20, showLoading: false });
        }
    }
    ...
    syncStyleEditorControls() {
        ...
        ['style-bold', 'style-italic', 'style-underline', 'style-reset', 'style-open-inline-editor', 'style-toggle-move'].forEach((action) => {
    ...
    refreshVisibleHotspotSelection() {
        ...
        this.querySelectorAll('[data-style-hotspot="1"]').forEach((zone) => {
            ...
            const isDraggable = Boolean(isActive && this.moveModeEnabled);
            zone.classList.toggle('is-draggable', isDraggable);
            zone.style.touchAction = isDraggable ? 'none' : 'auto';
            if (!isDraggable) {
    ...
    syncVisibleHotspotsWithOffsets(options = {}) {
        const skipSelectionRefresh = Boolean(options?.skipSelectionRefresh);
        ...
        if (activeMoved && !skipSelectionRefresh) {
    ...
    adjustCutForBlocks(cutY, startY, blockRanges = [], maxSliceHeightPx = 0) {
        ...
        let guard = 0;
        const maxFallbackGapPx = Math.max(110, Math.floor((Number(maxSliceHeightPx) || 0) * 0.15));
        ...
        while (guard < 30) {
            ...
            const movedUpPx = Math.max(0, originalCut - nextCut);

            // Si l'ajustement cree un grand vide en bas de page, on garde la coupe initiale
            // pour privilegier une pagination visuellement dense.
            if (maxFallbackGapPx > 0 && movedUpPx > maxFallbackGapPx) {
                adjustedCut = originalCut;
                break;
            }
    ...
    appendPageHotspots(layer, options) {
        ...
        hotspots.forEach((spot) => {
            ...
            zone.className = 'pv-hotspot absolute z-[2] cursor-pointer bg-sky-300/0';
            if (spot.styleKey && spot.styleKey === this.activeStyleTargetKey) {
                zone.className += ' ring-2 ring-sky-500 ring-inset bg-sky-300/20 is-active';
            }
    ...
    truncateCanvasLineWithEllipsis(ctx, line, maxWidthPx) {
        const raw = String(line || '').trim();
        if (!raw) return '';
        if (ctx.measureText(raw).width <= maxWidthPx) return raw;
        const suffix = '...';
        let trimmed = raw;
        while (trimmed.length > 1 && ctx.measureText(`${trimmed}${suffix}`).width > maxWidthPx) {
            trimmed = trimmed.slice(0, -1);
        }
        return `${trimmed}${suffix}`;
    }

    buildCanvasMetaLines(ctx, text, maxWidthPx, maxLines = 2) {
        const source = this.normalizePageMetaText(text);
        if (!source) return [];
        ...
        hardLines.forEach((hardLine) => {
            ...
            for (let i = 1; i < words.length; i += 1) {
                ...
                pushLine(this.truncateCanvasLineWithEllipsis(ctx, current, maxWidthPx));
                if (overflow) break;
                current = words[i];
            }
            if (!overflow) {
        ...
        if (overflow) {
            ...
            lines[last] = this.truncateCanvasLineWithEllipsis(ctx, lines[last], maxWidthPx);
        }
        return lines.slice(0, maxLines);
    }

    drawPageMetaOnCanvas(pageCtx, options) {
        if (!pageCtx) return;
        const {
            pageCanvasWidthPx = 1,
            pageCanvasHeightPx = 1,
        ...
        const headerText = this.getPageMetaText('header');
        const footerText = this.getPageMetaText('footer');
        if (!headerText && !footerText) return;

        const safeMargin = Math.max(0, Number(marginPx) || 0);
        const leftPx = safeMargin;
        const widthPx = Math.max(80, Math.min(pageCanvasWidthPx - leftPx, drawWidthPx));
        const centerX = leftPx + (widthPx / 2);
        const maxTextWidth = Math.max(70, widthPx - 18);
        const fontFamily = this.getTypographyConfig(this.typographyKey)?.fontFamily || '"Poppins", sans-serif';
        const fontSizePx = Math.max(9, Math.round(Math.min(12, widthPx / 65)));
        const lineHeightPx = Math.max(12, Math.round(fontSizePx * 1.22));
        const topPaddingPx = safeMargin > 0 ? Math.max(6, safeMargin - 1) : 8;
        const bottomPaddingPx = safeMargin > 0 ? Math.max(6, safeMargin - 1) : 8;

        pageCtx.save();
        pageCtx.fillStyle = '#334155';
        pageCtx.font = `500 ${fontSizePx}px ${fontFamily}`;
        pageCtx.textAlign = 'center';
        pageCtx.textBaseline = 'top';

        if (headerText) {
            const headerLines = this.buildCanvasMetaLines(pageCtx, headerText, maxTextWidth, 2);
            headerLines.forEach((line, index) => {
                pageCtx.fillText(line, centerX, topPaddingPx + (index * lineHeightPx), maxTextWidth);
            });
        }

        if (footerText) {
            const footerLines = this.buildCanvasMetaLines(pageCtx, footerText, maxTextWidth, 2);
            const startY = Math.max(
                4,
                pageCanvasHeightPx - bottomPaddingPx - (footerLines.length * lineHeightPx)
            );
            footerLines.forEach((line, index) => {
                pageCtx.fillText(line, centerX, startY + (index * lineHeightPx), maxTextWidth);
            });
        }
        pageCtx.restore();
    }

    appendPageMetaHotspots(layer, options) {
        if (!layer) return;
        const {
            pageCanvasWidthPx = 1,
            pageCanvasHeightPx = 1,
        ...
        const safeMargin = Math.max(0, Number(marginPx) || 0);
        const zoneHeightPx = Math.max(24, safeMargin > 0 ? safeMargin + 16 : 28);
        const leftPx = safeMargin;
        const widthPx = Math.max(60, Math.min(drawWidthPx, pageCanvasWidthPx - leftPx));

        const createZone = (type, topPx) => {
            const zone = document.createElement('button');
            zone.type = 'button';
            zone.className = 'absolute z-[4] cursor-text border border-dashed border-sky-400/0 bg-sky-200/0 transition-colors duration-150 hover:border-sky-400/55 hover:bg-sky-200/12';
            zone.style.left = `${(leftPx / pageCanvasWidthPx) * 100}%`;
            zone.style.top = `${(topPx / pageCanvasHeightPx) * 100}%`;
            zone.style.width = `${(widthPx / pageCanvasWidthPx) * 100}%`;
            zone.style.height = `${(zoneHeightPx / pageCanvasHeightPx) * 100}%`;
            zone.setAttribute('data-page-meta-hotspot', '1');
            zone.setAttribute('data-page-meta-type', type);
            zone.setAttribute('aria-label', type === 'header' ? 'Editer le header de page' : 'Editer le footer de page');
            zone.title = type === 'header'
                ? 'Cliquer pour editer le header de page'
                : 'Cliquer pour editer le footer de page';
            layer.appendChild(zone);
        };

        createZone('header', 0);
        createZone('footer', Math.max(0, pageCanvasHeightPx - zoneHeightPx));
    }

    trimPdfLineWithEllipsis(pdf, line, maxWidthMm) {
        const raw = String(line || '').trim();
        if (!raw) return '';
        if (pdf.getTextWidth(raw) <= maxWidthMm) return raw;
        const suffix = '...';
        let trimmed = raw;
        while (trimmed.length > 1 && pdf.getTextWidth(`${trimmed}${suffix}`) > maxWidthMm) {
            trimmed = trimmed.slice(0, -1);
        }
        return `${trimmed}${suffix}`;
    }

    buildPdfMetaLines(pdf, text, maxWidthMm, maxLines = 2) {
        const source = this.normalizePageMetaText(text);
        if (!source || !pdf || typeof pdf.splitTextToSize !== 'function') return [];
        const lines = pdf
            .splitTextToSize(source, maxWidthMm)
        ...
        limited[lastIndex] = this.trimPdfLineWithEllipsis(pdf, limited[lastIndex], maxWidthMm);
        return limited;
    }
    ...
    async renderPaginatedPreview(options = {}) {
        ...
        try {
            ...
            for (const sliceHeightPx of slices) {
                ...
                let currentPageCanvasHeightPx = pageCanvasHeightPx;
                if (isLastPage) {
                    const clampedAdjust = this.clampLastPageHeightAdjust(
                        this.lastPageHeightAdjustPx,
                        pageCanvasHeightPx,
                        marginPx
                    );
                    this.lastPageHeightAdjustPx = clampedAdjust;
                    currentPageCanvasHeightPx = pageCanvasHeightPx + clampedAdjust;
                }

                const pageCanvas = document.createElement('canvas');
                pageCanvas.width = canvas.width;
                pageCanvas.height = currentPageCanvasHeightPx;
                ...
                this.drawPageMetaOnCanvas(pageCtx, {
                ...
                pageWrap.className = 'pv-page-shell flex flex-col items-center gap-1';
                const pageLabel = document.createElement('p');
                pageLabel.className = 'pv-page-label text-[11px] font-semibold uppercase tracking-wide';
                pageLabel.textContent = `Page ${pageNumber}`;
                const pageFrame = document.createElement('div');
                pageFrame.className = 'pv-page-frame w-[254px] sm:w-[349px] md:w-[476px] lg:w-[635px] xl:w-[794px]';
                pageFrame.setAttribute('data-page-frame', '1');
                pageFrame.dataset.pageCanvasWidth = String(pageCanvas.width);
                pageFrame.dataset.pageCanvasHeight = String(pageCanvas.height);
                pageFrame.dataset.basePageCanvasHeight = String(pageCanvasHeightPx);
                const pageImg = document.createElement('img');
                pageImg.src = pageCanvas.toDataURL('image/png', 1.0);
                pageImg.alt = `Previsualisation page ${pageNumber}`;
                pageImg.className = 'block h-auto w-full';
                ...
                this.appendPageMetaHotspots(hotspotLayer, {
                    pageCanvasWidthPx: pageCanvas.width,
                    pageCanvasHeightPx: pageCanvas.height,
                    marginPx,
                    drawWidthPx
                ...
                if (isLastPage) {
                    ...
                    resizeHandle.className = 'pv-last-resize-handle';
                    resizeHandle.setAttribute('data-last-page-resize-handle', '1');
                    resizeHandle.setAttribute('aria-label', 'Redimensionner la derniere page');
                    pageFrame.appendChild(resizeHandle);
                }
    ...
    renderExperienceList(items) {
        ...
        if (items.length > 1) {
            items.slice(1).forEach((item) => {
                ...
                date.className = 'pv-exp-date w-[110px] shrink-0 cursor-pointer text-[14px] font-semibold hover:opacity-90';
                date.setAttribute('data-edit-step', 'experiences');
                date.setAttribute('data-edit-block', '');
                date.setAttribute('data-autofit', '');
                date.setAttribute('data-autofit-min', '8');
                date.textContent = item.dates || 'Periode';

                const content = document.createElement('div');
                content.className = 'pv-exp-content flex-1';

                const roleLine = document.createElement('p');
                roleLine.className = 'pv-exp-role m-0 cursor-pointer break-words text-[14px] font-semibold uppercase hover:opacity-90';
                roleLine.setAttribute('data-edit-step', 'experiences');
                roleLine.setAttribute('data-edit-block', '');
                roleLine.setAttribute('data-autofit', '');
                roleLine.setAttribute('data-autofit-min', '8');
                roleLine.textContent = item.title || 'Poste';

                const company = document.createElement('p');
                company.className = 'pv-exp-company m-0 mt-0.5 cursor-pointer break-words text-[14px] font-medium hover:opacity-90';
                company.setAttribute('data-edit-step', 'experiences');
                company.setAttribute('data-edit-block', '');
                company.setAttribute('data-autofit', '');
                company.setAttribute('data-autofit-min', '8');
                company.textContent = `${item.organization || 'Entreprise'} - ${item.location || 'Lieu'}`;

                const bulletList = document.createElement('ul');
                bulletList.className = 'pv-exp-list mb-0 mt-2 list-disc pl-[18px] text-[14px] leading-[1.5]';
                bulletList.setAttribute('data-edit-step', 'experiences');
                bulletList.setAttribute('data-edit-block', '');
                bulletList.setAttribute('data-autofit', '');
                bulletList.setAttribute('data-autofit-min', '8');

                content.appendChild(roleLine);
                content.appendChild(company);
                content.appendChild(bulletList);
    ...
    async downloadPdf(button) {
        ...
        try {
            ...
            const headerLines = this.buildPdfMetaLines(pdf, this.getPageMetaText('header'), Math.max(40, contentWidthMm - 6), 2);
            const footerLines = this.buildPdfMetaLines(pdf, this.getPageMetaText('footer'), Math.max(40, contentWidthMm - 6), 2);
            let renderedPx = 0;
            let pageIndex = 0;
            for (const sliceHeightPx of slices) {
                const sliceCanvas = document.createElement('canvas');
                ...
                if (headerLines.length) {
                    const headerYmm = marginMm > 0 ? Math.max(3.5, marginMm - 2.2) : 4.2;
                    pdf.text(headerLines, centerXmm, headerYmm, { align: 'center' });
                }
                if (footerLines.length) {
                    const footerLineGapMm = 3.8;
                    const footerStartYmm = pageHeightMm - (marginMm > 0 ? Math.max(3.8, marginMm - 1.2) : 4.2) - ((footerLines.length - 1) * footerLineGapMm);
                    pdf.text(footerLines, centerXmm, footerStartYmm, { align: 'center' });
                }
